...
await db.batch(
  ['DELETE FROM journal_entries', 'DELETE FROM trades', 'DELETE FROM accounts'],
  'write'
);

const accountResult = await db.execute({
  sql: 'INSERT INTO accounts (name, broker, starting_balance, currency) VALUES (?, ?, ?, ?)',
  args: ['Main Account', 'Demo Broker', 10000, 'USD']
});
const accountId = Number(accountResult.lastInsertRowid);
const secondAccount = await db.execute({
  sql: 'INSERT INTO accounts (name, broker, starting_balance, currency) VALUES (?, ?, ?, ?)',
  args: ['Crypto Wallet', '', 5000, 'USD']
});
...
const symbols = [
  ...
  { name: 'BTCUSD', base: 64000, decimals: 1 },
  { name: 'ETHUSD', base: 3200, decimals: 2 },
...
for (let i = 0; i < 3; i++) {
  const sym = pick(symbols.filter((s) => s.crypto === undefined));
...
for (const [date, title, content, mood] of journalEntries) {
  await db.execute({
    sql: 'INSERT INTO journal_entries (date, title, content, mood) VALUES (?, ?, ?, ?)',
    args: [date, title, content, mood]
  });
}

console.log(`Seeded ${tradeCount} trades, 2 accounts, ${journalEntries.length} journal entries.`);
